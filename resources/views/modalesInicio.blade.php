                        ...
                        <p>
                            @foreach ($islas as $isla)
                                <button class="btn btn-primary" type="button" data-toggle="collapse"
                                    data-target="#collapseExample{{ $isla->isla }}" aria-expanded="false"
                                    ...
                                    @foreach ($maquinas as $maquina)
                                        @if ($maquina->isla == $isla->isla)
                                            @if ($maquina->estatus == 'O' || $maquina->estatus == 'M')
                                                <fieldset disabled>
                                                    <div class="col-3">
                                                        <button class="boton-maquina"><i class="bi bi-pc-display"
                                                                style="font-size: 4rem; @if ($maquina->estatus == 'M') color: gray; @elseif($maquina->estatus == 'O') color:red; @endif"></i></button>
                                                    </div>
                                                </fieldset>
                                            @else
                                                <div class="col-3">
                                                    <button class="boton-maquina"
                                                        onclick="valoresRenta({{ $isla->isla }},{{ $maquina->id }}), seleccionada(this)"><i
                                                            class="bi bi-pc-display maquina-icon @if ($maquina->estatus == 'D') maquina-disponible @endif"
                                                            style="font-size: 4rem;"></i></button>
                                                </div>
                                            @endif
                                        @endif
                                    @endforeach
                    ...
                    <div class="card card-body">
                        <form action="{{ route('rentarmaquina') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 pt-2">
                                    <div class="form-group">
                                        <label for="usuarios" style="font-size: medium; color:black;">Seleccione un
                                            estudiante</label>
                                    </div>
                                ...
                                <div class="col-12 pt-3">
                                    <div class="form-group">
                                        <label for="maquina" style="font-size: medium; color:black;">Maquina a
                                            rentar</label>
                                        <fieldset disabled>
                                            <input type="text" class="form-control" id="maquinaVista"
                                                name="maquinaVista" placeholder="Seleccione una maquina"
                                                value="{{ 'Isla ' . old('isla_id') . '- Maquina ' . old('maquina_id') }}">
                                        </fieldset>
                                        <input type="text" class="form-control" id="maquinaForm" name="maquina_id"
                                            style="display: none;" value="{{ old('maquina_id') }}">
                                        <input type="text" class="form-control" id="islaForm" name="isla_id"
                                            style="display: none;" value="{{ old('isla_id') }}">
                                        @error('maquina_id')
                                            <br>
...
@foreach ($rentas as $renta)
    <div class="modal fade" id="createModal{{ $renta->id }}" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('rentaMaquina.update', $renta->id) }}" method="POST"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <input type="text" name="maquina_id" value="{{ $renta->maquina_id }}"
                            style="display: none;">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
...
<div class="modal fade" id="modal-libros" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <form action="{{ route('renta-libros.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body row">
                        ...
                        <select id="libros_id" name="libros_id">
                            ...
                            @foreach ($librosDisponibles as $libro)
                                <option value="{{ $libro->id }}">{{ $libro->titulo }} - {{ $libro->genero }} -
                                    {{ $libro->disponibilidad ? 'Disponible' : 'No disponible' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12 pt-3">
                    ...
                    <div class="col-12 pt-1">
                        ...
                        <select class="form-control" id="alumnos-libro" name="alumnos-libro">
                            <option value="">Seleccionar estudiante...</option>
                        </select>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="notas" name="notas" rows="5"
                                placeholder="Ingrese el estado en el que se encuentra el libro antes del prestamo:"></textarea>
                        </div>
                    </div>
